# Z-WBE 21-Day Campaign: X (Twitter) - Day 15
...
```markdown
Why can't electron microscopes just scan faster?
Shot noise.
...
```markdown
4nm isotropic = 18.75 Petavoxels.
...
- **Mentions**: None
- **Claims Verified**: YES - Sensitivity tests pass in 40ms
- **Manual Review Required**: NO
...
```markdown
Chemical fixation causes tissue shrinkage: extracellular space drops from 20% to <5%.
...
```markdown
You cannot speed up the electron beam without vaporizing the tissue block.
