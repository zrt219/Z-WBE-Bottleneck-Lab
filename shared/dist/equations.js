...
exports.calculateVoxelCount = calculateVoxelCount;
exports.calculateRawDataBytes = calculateRawDataBytes;
exports.calculateEffectiveImagingThroughput = calculateEffectiveImagingThroughput;
exports.calculateAcquisitionTimeYears = calculateAcquisitionTimeYears;
exports.calculateModelStateBytes = calculateModelStateBytes;
exports.calculateComputeDemandFlops = calculateComputeDemandFlops;
exports.calculateMemoryTrafficBytesSec = calculateMemoryTrafficBytesSec;
exports.calculateInterconnectTrafficBytesSec = calculateInterconnectTrafficBytesSec;
exports.calculateAllMetrics = calculateAllMetrics;
exports.formatBytes = formatBytes;
exports.formatComputeFlops = formatComputeFlops;
exports.formatPowerDemand = formatPowerDemand;
exports.formatBandwidth = formatBandwidth;
exports.formatAcquisitionDuration = formatAcquisitionDuration;
 ...
 * N_voxel = V / (dx * dy * dz)
 */
function calculateVoxelCount(tissueVolumeMm3, voxelResXNm, voxelResYNm, voxelResZNm) {
 ...
 */
function calculateRawDataBytes(voxelCount, bitsPerVoxel) {
 ...
 */
function calculateEffectiveImagingThroughput(ratePerMachineMm3Year, machineCount, utilization) {
 ...
 */
function calculateAcquisitionTimeYears(tissueVolumeMm3, effectiveThroughputMm3Year) {
 ...
 */
function calculateModelStateBytes(neuronCount, bytesPerNeuron, synapseCount, bytesPerSynapse) {
 ...
 */
function calculateComputeDemandFlops(neuronCount, neuronUpdateRateHz, opsPerNeuronUpdate, synapseCount, averageFiringRateHz, opsPerSynapticEvent) {
    const neuralFlops = Math.max(0, neuronCount) *
 ...
 */
function calculateMemoryTrafficBytesSec(neuronCount, neuronUpdateRateHz, bytesPerNeuron, synapseCount, averageFiringRateHz, bytesPerSynapse) {
    const neuralTrafficBytesSec = Math.max(0, neuronCount) *
        Math.max(0, neuronUpdateRateHz) *
        Math.max(0, bytesPerNeuron);
    const synapticTrafficBytesSec = Math.max(0, synapseCount) *
 ...
 */
function calculateInterconnectTrafficBytesSec(synapseCount, averageFiringRateHz, crossNodeFraction = 0.25, spikePacketBytes = 8) {
 ...
 */
function calculateAllMetrics(assumptions) {
    const { acquisition, reconstruction, neuralModel, economics } = assumptions;
    // 1. Acquisition
    const voxelCount = calculateVoxelCount(acquisition.tissueVolumeMm3, acquisition.voxelResXNm, acquisition.voxelResYNm, acquisition.voxelResZNm);
    ...
    const effectiveImagingThroughputMm3Year = calculateEffectiveImagingThroughput(acquisition.imagingRatePerMachineMm3Year, acquisition.machineCount, acquisition.utilization);
    const acquisitionTimeYears = calculateAcquisitionTimeYears(acquisition.tissueVolumeMm3, effectiveImagingThroughputMm3Year);
    const acquisitionTimeDays = isFinite(acquisitionTimeYears)
        ? acquisitionTimeYears * 365.25
        : Infinity;
    // 2. Reconstruction
    const automatedReconstructionYears = reconstruction.automatedThroughputMm3Year > 0
    ...
    const baseProofreadingHours = acquisition.tissueVolumeMm3 * reconstruction.manualProofreadingHoursPerMm3;
    ...
    const manualProofreadingPersonHours = (baseProofreadingHours * (errorRate / 0.02)) / proofreadingMultiplier;
    const manualProofreadingPersonYears = manualProofreadingPersonHours / 2000; // 2000 hrs/work-year
    // 3. Model state & Compute
    const modelStateBytes = calculateModelStateBytes(neuralModel.neuronCount, neuralModel.bytesPerNeuron, neuralModel.synapseCount, neuralModel.bytesPerSynapse);
    const modelStateTb = modelStateBytes / BYTES_PER_TB;
    const computeDemandFlops = calculateComputeDemandFlops(neuralModel.neuronCount, neuralModel.neuronUpdateRateHz, neuralModel.computeOpsPerNeuronUpdate, neuralModel.synapseCount, neuralModel.averageFiringRateHz, neuralModel.computeOpsPerSynapticEvent);
    const computeDemandPflops = computeDemandFlops / 1e15;
    const { neuralTrafficBytesSec, synapticTrafficBytesSec, totalMemoryTrafficBytesSec } = calculateMemoryTrafficBytesSec(neuralModel.neuronCount, neuralModel.neuronUpdateRateHz, neuralModel.bytesPerNeuron, neuralModel.synapseCount, neuralModel.averageFiringRateHz, neuralModel.bytesPerSynapse);
    const memoryTrafficTbS = totalMemoryTrafficBytesSec / BYTES_PER_TB;
    const interconnectTrafficBytesSec = calculateInterconnectTrafficBytesSec(neuralModel.synapseCount, neuralModel.averageFiringRateHz);
    ...
    const imagingCostUsd = acquisition.machineCount * economics.imagingInstrumentCostPerYear * activeImagingYears;
    ...
    const energyCostUsdPerYear = totalPowerDemandMw * 8760 * economics.energyCostPerMwh;
    const proofreadingCostUsd = manualProofreadingPersonHours * economics.humanProofreadingHourlyRate;
    const totalEstimatedCostUsd = imagingCostUsd +
...
function formatComputeFlops(flops) {
    ...
    if (flops >= 1e3)
        return `${(flops / 1e3).toFixed(1)} kFLOPS`;
    return `${flops.toFixed(0)} FLOPS`;
}
function formatPowerDemand(mw) {
    if (!isFinite(mw) || mw <= 0)
        return '0 W';
    if (mw >= 1.0)
...
function formatBandwidth(tbS) {
    if (!isFinite(tbS) || tbS <= 0)
        return '0 B/s';
    if (tbS >= 1.0)
        return `${tbS.toFixed(2)} TB/s`;
    if (tbS >= 0.001)
        return `${(tbS * 1000).toFixed(1)} GB/s`;
    return `${(tbS * 1e6).toFixed(1)} MB/s`;
}
function formatAcquisitionDuration(years, days) {
    if (!isFinite(years) || years <= 0)
        return '0 hrs';
    if (years >= 1.0)
        return `${years.toFixed(2)} yrs (${Math.round(days).toLocaleString()} d)`;
    if (days >= 1.0)
        return `${days.toFixed(1)} days (${(days * 24).toFixed(0)} hrs)`;
    const hours = days * 24;
    if (hours >= 1.0)
