 ...
 */
function scenarioHash(model, promptVersion, assumptions, metrics) {
 ...
 */
function buildNemotronInputSchema(assumptions, metrics, bottleneck, sensitivity) {
    const limitations = [
...
exports.buildGroundingRequest = buildNemotronInputSchema;
/**
 ...
 */
function generateGroundedFallback(request, status = 'unavailable', errorMessage) {
...
function repairAndParseNemotronResponse(rawContent, modelIdentifier = 'nvidia/nemotron-3-super-120b-a12b:free', request) {
    ...
    const whatLimitsThisScenario = extractSection('WHAT LIMITS THIS SCENARIO\\??', [
        'WHY\\??',
        'WHAT IMPROVEMENT MATTERS MOST\\??',
        'WHAT DOES NOT HELP MUCH\\??',
        'WHERE DID THE BOTTLENECK MOVE\\??',
    ...
    const whatImprovementMattersMost = extractSection('WHAT IMPROVEMENT MATTERS MOST\\??', [
        'WHAT DOES NOT HELP MUCH\\??',
        'WHERE DID THE BOTTLENECK MOVE\\??',
        'WHAT REMAINS UNCERTAIN\\??',
        'UNCERTAINTIES',
    ...
    const whatDoesNotHelpMuch = extractSection('WHAT DOES NOT HELP MUCH\\??', [
        'WHERE DID THE BOTTLENECK MOVE\\??',
        'WHAT REMAINS UNCERTAIN\\??',
        'UNCERTAINTIES',
        'WHAT NEEDS REAL EXPERIMENTAL EVIDENCE\\??',
    ...
    const whereDidTheBottleneckMove = extractSection('WHERE DID THE BOTTLENECK MOVE\\??', [
        'WHAT REMAINS UNCERTAIN\\??',
        'UNCERTAINTIES',
        'WHAT NEEDS REAL EXPERIMENTAL EVIDENCE\\??',
        'WHAT WOULD NEED EMPIRICAL VALIDATION\\??'
    ]) || 'Bottleneck remains steady on active constraint.';
    const whatRemainsUncertain = extractSection('WHAT REMAINS UNCERTAIN\\??', [
        'WHAT NEEDS REAL EXPERIMENTAL EVIDENCE\\??',
        'WHAT WOULD NEED EMPIRICAL VALIDATION\\??'
    ]) ||
