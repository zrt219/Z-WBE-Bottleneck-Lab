...
exports.calculateBottlenecks = calculateBottlenecks;
...
function calculateBottlenecks(assumptions, metrics) {
    ...
    const computeRatio = computePflops > 0 ? metrics.computeDemandPflops / computePflops : 9999;
    ...
    const memoryRatio = memoryBandwidth > 0 ? metrics.memoryTrafficTbS / memoryBandwidth : 9999;
    ...
    const interconnectRatio = interconnectBandwidth > 0 ? metrics.interconnectTrafficTbS / interconnectBandwidth : 9999;
    ...
    const powerRatio = powerBudget > 0 ? metrics.totalPowerDemandMw / powerBudget : 9999;
    const powerScore = powerRatio * 100;
    // 8. Economic Cost Pressure: Estimated cost vs budget ceiling USD
    const costRatio = economics.budgetCeilingUsd > 0
    ...
    const pressures = {
        ACQUISITION: {
            ...
            demandFormatted: (0, equations_1.formatAcquisitionDuration)(metrics.acquisitionTimeYears, metrics.acquisitionTimeDays),
            capacityFormatted: `${targetYears.toFixed(1)} yr target`,
            summary: `Takes ${metrics.acquisitionTimeYears >= 1 ? `${metrics.acquisitionTimeYears.toFixed(2)} years` : (0, equations_1.formatAcquisitionDuration)(metrics.acquisitionTimeYears, metrics.acquisitionTimeDays)} across ${acquisition.machineCount} instruments.`
        },
        RECONSTRUCTION: {
        ...
        STORAGE: {
            ...
            demandFormatted: (0, equations_1.formatBytes)(totalStorageBytes),
            capacityFormatted: `${storageCapacityPb} PB (${(0, equations_1.formatBytes)(storageCapacityBytes)})`,
            summary: `Image + model state requires ${(0, equations_1.formatBytes)(totalStorageBytes)}.`
        },
        COMPUTE: {
            score: Number(computeScore.toFixed(2)),
            rawRatio: Number(computeRatio.toFixed(3)),
            demandFormatted: (0, equations_1.formatComputeFlops)(metrics.computeDemandFlops),
            capacityFormatted: (0, equations_1.formatComputeFlops)(computePflops * 1e15),
            summary: `Real-time emulation demand is ${(0, equations_1.formatComputeFlops)(metrics.computeDemandFlops)}.`
        },
        MEMORY_BANDWIDTH: {
            score: Number(memoryScore.toFixed(2)),
            rawRatio: Number(memoryRatio.toFixed(3)),
            demandFormatted: (0, equations_1.formatBandwidth)(metrics.memoryTrafficTbS),
            capacityFormatted: (0, equations_1.formatBandwidth)(memoryBandwidth),
            summary: `Continuous state transfer requires ${(0, equations_1.formatBandwidth)(metrics.memoryTrafficTbS)}.`
        },
        INTERCONNECT: {
            score: Number(interconnectScore.toFixed(2)),
            rawRatio: Number(interconnectRatio.toFixed(3)),
            demandFormatted: (0, equations_1.formatBandwidth)(metrics.interconnectTrafficTbS),
            capacityFormatted: (0, equations_1.formatBandwidth)(hardware.interconnectBandwidthTbS ?? 2),
            summary: `Cross-node synchronization traffic is ${(0, equations_1.formatBandwidth)(metrics.interconnectTrafficTbS)}.`
        },
        POWER: {
            score: Number(powerScore.toFixed(2)),
            rawRatio: Number(powerRatio.toFixed(3)),
            demandFormatted: (0, equations_1.formatPowerDemand)(metrics.totalPowerDemandMw),
            capacityFormatted: (0, equations_1.formatPowerDemand)(hardware.powerBudgetMw ?? 10),
            summary: `System compute & network power reaches ${(0, equations_1.formatPowerDemand)(metrics.totalPowerDemandMw)}.`
        },
        ECONOMIC_COST: {
            score: Number(costScore.toFixed(2)),
            rawRatio: Number(costRatio.toFixed(3)),
            demandFormatted: (0, equations_1.formatCurrency)(metrics.totalEstimatedCostUsd),
            capacityFormatted: `${(0, equations_1.formatCurrency)(economics.budgetCeilingUsd)} budget`,
            summary: `Total pipeline financial cost is estimated at ${(0, equations_1.formatCurrency)(metrics.totalEstimatedCostUsd)}.`
        }
    };
    // Rank dimensions strictly deterministically
    const ranked = Object.keys(pressures).sort((a, b) => {
    ...
    if (assumptions.isHypotheticalHumanScale) {
        uncertaintyNotes.push('HYPOTHETICAL SCALE: Human-scale connectome and biophysical parameters represent theoretical scaling projections, not demonstrated empirical data.');
    }
    if (reconstruction.rawSegmentationAccuracy < 0.99) {
        uncertaintyNotes.push(`Segmentation accuracy at ${(reconstruction.rawSegmentationAccuracy * 100).toFixed(1)}% yields high proofreading multiplier sensitivity; small error increases create non-linear proofreading backlogs.`);
    }
    if (assumptions.neuralModel.bytesPerSynapse < 8) {
        uncertaintyNotes.push('Synaptic state allocation assumes minimal plasticity variables; complex multi-stage plastic states require substantially higher memory bandwidth.');
    }
    if (pressures.MEMORY_BANDWIDTH.score > 80 && pressures.COMPUTE.score < 50) {
        uncertaintyNotes.push('System demonstrates high arithmetic intensity shortfall (memory-bound execution); increasing raw compute FLOPs yields diminishing returns without memory subsystem scaling.');
    }
    const sensitivitySummary = dominantScore > 100
