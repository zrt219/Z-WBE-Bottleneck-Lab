...
export const TutorialPipelinePage: React.FC = () => {
  ...
  const calculationResults = useMemo(() => {
    ...
    const bottlenecks = calculateBottlenecks(metrics, activePreset);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-600 text-sm sm:text-base mt-2 leading-relaxed">
          ...
          an emulation is feasible if and only if all 8 dimensions remain below critical saturation ($P \le 1.0$).
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <div className="text-sm font-black text-amber-950">
                {calculationResults.bottlenecks.dominantBottleneck.title}
              </div>
            </div>
          </div>
          <div className="font-mono text-xs font-bold bg-white/80 px-3 py-1.5 rounded-lg border border-amber-300 text-amber-900">
            Severity Index: {calculationResults.bottlenecks.dominantBottleneck.score.toFixed(1)} / 10
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs font-mono text-slate-500">
            ...
            <span>Pressure Ratio (Demand ÷ Available Capacity)</span>
          </div>

          <div className="space-y-2.5">
            {calculationResults.bottlenecks.dimensions.map((dim) => {
              const isBottleneck = dim.isBottleneck;
              const pressurePct = Math.min(dim.pressureRatio * 100, 100);
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    <Tooltip id={`dim_${dim.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}`}>
                    ...
                    <div className="flex items-center space-x-2 font-mono">
                      ...
                      >
                        {(dim.pressureRatio).toFixed(2)}×
                      </span>
                    </div>
                  </div>

                  {/* Progress Meter with 1.0 Marker */}
